...
_RULES_CACHE: dict[str, Dict[str, Tuple[re.Pattern, Optional[re.Pattern], Optional[re.Pattern]]]] = {}
...
def build_rules(catalog: dict):
    ...
    rules: Dict[str, Tuple[re.Pattern, Optional[re.Pattern], Optional[re.Pattern]]] = {}
    for app in catalog["apps"]:
        aid = app["id"]
        kws = app.get("keywords", [])
        neg_kws = app.get("neg_keywords", [])
        req_kws = app.get("require_keywords", [])
        ...
        req_pat = r"\b(" + "|".join([re.escape(k) for k in req_kws]) + r")\b" if req_kws else None
        try:
        ...
        if req_pat:
            try:
                req_rx = re.compile(req_pat, re.I)
            except re.error:
                req_rx = re.compile("|".join([re.escape(k) for k in req_kws]), re.I)
        else:
            req_rx = None
        rules[aid] = (pos_rx, neg_rx, req_rx)
...
def score_rules(msg: str, rules: Dict[str, Tuple["re.Pattern", Optional["re.Pattern"], Optional["re.Pattern"]]]):
    out = {}
    for aid, (pos_rx, neg_rx, req_rx) in rules.items():
        if neg_rx and neg_rx.search(msg):
            out[aid] = 0.0
            continue
        if pos_rx.search(msg):
            if req_rx is None or req_rx.search(msg):
                out[aid] = 0.95
            else:
                # Domain guard: matched app keyword but missing required domain words → low score
                out[aid] = 0.2
        else:
            out[aid] = 0.0
    return out
...
def calibrated_decision(msg: str, catalog_path: str | Path, pre: Optional[Dict[str, Any]] = None):
    raw_msg = msg
    msg = normalize_text(msg)
    if pre and pre.get("catalog") is not None and pre.get("rules") is not None:
        catalog = pre["catalog"]
        rules = pre["rules"]
    else:
    ...
    # Embed similarity to app corpora (use preloaded if available)
    if pre and all(pre.get(k) is not None for k in ("embedder", "app_ids", "embs")):
        embedder = pre["embedder"]
        app_ids = pre["app_ids"]
        embs = pre["embs"]
    else:
    ...
    # Confidence margin guard: avoid routing when top-1 vs top-2 are too close
    margin_guard = float(os.getenv("ROUTER_MARGIN_GUARD", 0.08))
    if len(sorted_apps) >= 2 and (best_score - second_score) < margin_guard:
        return {
            "type": "SAFE_REPLY",
